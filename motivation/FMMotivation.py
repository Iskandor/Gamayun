...
class FMMotivationWithSemanticLoss:
    def __init__(self, network, loss, lr, eta=1, device='cpu'):
        ...
        params_to_train = list(self._network.ppo_encoder.parameters())
        params_to_train += list(self._network.forward_model.parameters())
        if hasattr(self._network, 'noise_generator'):
            params_to_train += list(self._network.noise_generator.parameters())
        if hasattr(self._network, 'hidden_model'):
            params_to_train += list(self._network.hidden_model.parameters())
        if hasattr(self._network, 'inverse_model'):
            params_to_train += list(self._network.inverse_model.parameters())

        self._optimizer = torch.optim.Adam(params_to_train, lr=lr)
