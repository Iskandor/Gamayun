...
class ExperimentNEnvPPO:
    ...
    def run_aspd_model(self, agent, trial):
        ...
        while step_counter.running():
            ...
            ext_reward = torch.tensor(reward, dtype=torch.float32)
            error, int_reward = agent.motivation.reward(state0, action0)
            error = error.cpu()
            int_reward = int_reward.cpu().clip(0.0, 1.0)
            ...
            aspd_state = agent.network.aspd_model.preprocess(state0)
            analytic.update(re=ext_reward,
                            ri=int_reward,
                            ext_value=value[:, 0].unsqueeze(-1).cpu(),
                            int_value=value[:, 1].unsqueeze(-1).cpu(),
                            error=error,
                            state_space=aspd_state.norm(p=2, dim=[1, 2, 3]).unsqueeze(-1).cpu(),
                            feature_space=features.norm(p=2, dim=1, keepdim=True).cpu())
