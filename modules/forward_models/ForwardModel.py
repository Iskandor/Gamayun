...
class ForwardModelType(Enum):
    ...
    ForwardModelSkipConnectionMultiStep = 8
...
def chooseModel(config, forward_model_type, action_multiplier = 1):
    ...
    elif forward_model_type == ForwardModelType.ForwardModelSkipConnectionMultiStep:
        forward_model = ForwardModelSkipConnectionMultiStep(config)
    else:
        forward_model = ForwardModel(config)
...
class ForwardModelSkipConnectionMultiStep(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.action_dim = config.action_dim
        self.feature_dim = config.feature_dim
