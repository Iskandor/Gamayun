...
from modules.atari.PPOAtariNetworkSNDv3 import PPOAtariNetworkSNDv3
from motivation.SNDMotivation import SNDMotivation


class PPOAtariSNDv3Agent(PPOAtariAgent):
    def __init__(self, config):
        super().__init__(config)
        self.model = PPOAtariNetworkSNDv3(config).to(config.device)
        self.motivation = SNDMotivation(self.model, SNDv3Loss(config, self.model), config.motivation_lr, config.motivation_scale, config.device)
    ...
    def _train(self, memory, indices):
        ...
        print("SNDv3 agent training: trajectory {0:d} batch size {1:d} epochs {2:d} training time {3:.2f}s".format(self.config.trajectory_size, self.config.batch_size, self.config.ppo_epochs, end - start))
