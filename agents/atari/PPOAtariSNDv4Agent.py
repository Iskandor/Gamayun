...
from loss.SNDv4Loss import SNDv4Loss
from modules.atari.PPOAtariNetworkSNDv4 import PPOAtariNetworkSNDv4
from motivation.SNDMotivation import SNDMotivation


class PPOAtariSNDv4Agent(PPOAtariAgent):
    def __init__(self, config):
        super().__init__(config)
        self.model = PPOAtariNetworkSNDv4(config).to(config.device)
        self.motivation = SNDMotivation(self.model, SNDv4Loss(config, self.model), config.motivation_lr, config.motivation_scale, config.device)
    ...
    def _train(self, memory, indices):
        ...
        print("SNDv4 agent training: trajectory {0:d} batch size {1:d} epochs {2:d} training time {3:.2f}s".format(self.config.trajectory_size, self.config.batch_size, self.config.ppo_epochs, end - start))
