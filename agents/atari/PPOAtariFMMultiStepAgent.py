...
from loss.FMLoss import STDIMMultiStepLoss
from modules.atari.PPOAtariFMNetwork import PPOAtariSTDIMLinearNoiseNetwork, PPOAtariSTDIMMultiStepNetwork
from motivation.FMMotivation import FMMultiStepMotivation
from modules.PPO_Modules import ActivationStage
from modules.forward_models.ForwardModel import ForwardModelType

class PPOAtariFMMultiStepAgent(PPOAtariAgent):
    def __init__(self, config, forward_model_type=ForwardModelType.ForwardModelSkipConnection, type=0):
        super().__init__(config)
        
        if type == 1:
            self.model = PPOAtariSTDIMMultiStepNetwork(config, forward_model_type).to(config.device)
        else:
            self.model = PPOAtariSTDIMLinearNoiseNetwork(config, forward_model_type).to(config.device)

      
        self.motivation = FMMultiStepMotivation(self.model,
                                       STDIMMultiStepLoss(self.model,
                                           self.model.ppo_encoder.hidden_size,
                                           self.model.ppo_encoder.local_layer_depth,
                                           config.device),
                                       config.motivation_lr,
                                       config.motivation_horizon,
                                       config.eta,
                                       config.device)
